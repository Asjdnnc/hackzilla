  ...
  useEffect(() => {
    const processScan = async () => {
      ...
      try {
        ...
        // First try to parse the QR data to get the teamId
        let teamId;
        try {
        ...
        if (actionResponse.data.data) {
          setScannedTeam(actionResponse.data.data);
          setShowResult(true);

          if (currentAction === 'check-in') {
            showMessage(`Team "${actionResponse.data.data.name}" checked in successfully!`, "success");
          } else if (['lunch', 'dinner', 'snacks'].includes(currentAction)) {
            const mealStatus = actionResponse.data.data.foodStatus?.[currentAction] || 'invalid';
            showMessage(`${currentAction.charAt(0).toUpperCase() + currentAction.slice(1)} status set to ${mealStatus.toUpperCase()} for team "${actionResponse.data.data.name}"!`, "success");
          } else if (currentAction === 'allotment') {
            const allotmentStatus = actionResponse.data.data.allotment || 'invalid';
            showMessage(`Allotment status set to ${allotmentStatus.toUpperCase()} for team "${actionResponse.data.data.name}"!`, "success");
          }
        } else {
          // If no data in response, fetch the team details
          const teamResponse = await axios.get(`${API_URL}/api/teams/${teamId}`, config);
          setScannedTeam(teamResponse.data.data);
          setShowResult(true);
          showMessage(`Scan processed successfully for team "${teamResponse.data.data.name}"!`, "success");
        }
      } catch (error) {
        const errorMessage = error.response?.data?.message || error.message || "An unexpected error occurred.";
        const errorStatus = error.response?.status;

        // If we have data in the error response, it's actually a success
        if (error.response?.data?.data) {
          setScannedTeam(error.response.data.data);
          setShowResult(true);
          showMessage(`Scan processed successfully for team "${error.response.data.data.name}"!`, "success");
          return;
        }
